...
Cloudflare Worker that backs the countdown demo. It handles **"Login with
GitHub"** and commits `countdown.json` to this repo on behalf of a **GitHub
App**. The Pages site sends the timestamps here with a session token; the Worker
...
browser ──POST {countdown:[…]} + Bearer <jwt>──▶ Worker ──App token──▶ Contents API
...
| `../index.html`, `../countdown.js`, `../style.css` | The site frontend (sign-in UI + countdown). |
| `../.github/workflows/append-log.yml` | Action that appends each commit to `countdown-log.json`. |
...
| `POST /` | Commit `countdown.json`. Requires a valid session whose login is allowed. |
...
| `FILE_PATH` | File to write (`countdown.json`). |
...
curl -s -X POST https://countdown.riverscape.workers.dev \
  -H 'Content-Type: application/json' -d '{"countdown":["x"]}'
